...
import { updateSale } from "@/services/dbService";
...
export default function UpdateSaleForm({ customers, sale }) {
  ...
  async function onSubmit(values) {
    try {
      const updateData = {
        id: sale.id, // Pass the current sale's ID
      ...
      toast(
        <pre className="mt-2 w-[340px] rounded-md bg-slate-950 p-4">
          <code className="text-white">
            {JSON.stringify("Sale updated successfully:", updatedSale, null, 2)}
          </code>
        </pre>
      );
    } catch (error) {
